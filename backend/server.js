...
app.get("/trip/:tripId", tripController.findTripById, (req, res) =>
  res.json(res.locals.trip)
);

// add a waypoint to a trip
app.post(
  "/trip/:tripId/addWaypoint",
...
app.post("/maps", mapsController.find, (req, res) => res.json(res.locals.maps));

/* TEST ROUTES */
app.get("/test", tripController.findAllTrips, (req, res) =>
  res.json(res.locals.trips)
);

app.post("/test", tripController.createTrip, (req, res) =>
  res.json(res.locals.trip)
);
